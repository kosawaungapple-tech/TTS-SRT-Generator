...
export class GeminiTTSService {
    ...
    const executeRequest = async (key: string) => {
      ...
      // Retry on certain errors: 429 (Rate Limit), 500 (Internal), 503 (High Demand/Service Unavailable), 504/502 (Gateway)
          if ((status === 429 || status === 500 || status === 503 || status === 504 || status === 502) && retryCount < MAX_RETRIES) {
            ...
            if (status === 429) {
              const retryMatch = errorMsg.match(/retry in ([\d.]+)s/i) || errorMsg.match(/after ([\d.]+)s/i) || errorMsg.match(/in ([\d.]+)s/i);
              let waitSeconds = 0;
              ...
              if (retryMatch && retryMatch[1]) {
                ...
                const innerDetails = details?.details || details;
                if (innerDetails && Array.isArray(innerDetails)) {
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const retryInfo = innerDetails.find((d: any) => d['@type']?.includes('RetryInfo') || d.retryDelay);
              ...
              if (waitSeconds > 0) {
                delay = (waitSeconds + 0.5) * 1000; // Add small buffer
                ...
                // Default backoff for 429 if no delay provided
                delay = Math.pow(2, retryCount) * 4000 + Math.random() * 1000;
                console.warn(`Gemini Proxy: Status ${status} (No delay). Retrying in ${Math.round(delay)}ms... (${retryCount + 1}/${MAX_RETRIES})`);
                if (onRetry) onRetry(Math.round(delay/1000), `API limit. Retrying in ${Math.round(delay/1000)}s...`);
              }
            } else {
  ...
  private async generateSingleTTS(text: string, config: TTSConfig, onRetry?: (seconds: number, message: string) => void): Promise<AudioResult> {
    ...
    try {
      ...
      if (cached) {
        ...
        if (cached.rawAudio) {
           const blob = new Blob([cached.rawAudio], { type: 'audio/wav' });
    ...
    // Convert Audio Blob to Base64 (consistent with WAV)
  ...
  async generateImage(prompt: string): Promise<string> {
    const body = {
      contents: [{ role: 'user', parts: [{ text: `Generate thumbnail: ${prompt}` }] }],
      generationConfig: {

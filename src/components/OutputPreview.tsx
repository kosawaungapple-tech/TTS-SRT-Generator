    ...
    if (audioRef.current) {
      try {
        if (isPlaying) {
          ...
          console.log("OutputPreview: Attempting to play audio...", result?.audioUrl);
          await audioRef.current.play();
          setIsPlaying(true);
        }
      } catch (err) {
        console.error("OutputPreview: Audio playback failed:", err);
        // If it's a DOMException (NotAllowedError), it's likely a browser restriction
        if (err instanceof DOMException && err.name === 'NotAllowedError') {
          console.warn("OutputPreview: Playback blocked by browser. User interaction required.");
        }
      }
    }
  };

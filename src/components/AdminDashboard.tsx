            ...
            <div className="space-y-6">
              <div className="bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6">
                ...
                <div className="flex items-center justify-between pb-6 border-b border-slate-200 dark:border-slate-800">
                  <div className="space-y-1">
                    <h4 className="text-sm font-bold text-rose-500 dark:text-rose-400 flex items-center gap-2">
                      <AlertCircle size={16} /> Maintenance Mode
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">ယာယီပြုပြင်ခြင်းကို ဖွင့်/ပိတ် ပြုလုပ်ရန်</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setGlobalSettings({ ...globalSettings, maintenance_mode: !globalSettings.maintenance_mode })}
                    className={`w-12 h-6 rounded-full transition-all relative shadow-inner ${globalSettings.maintenance_mode ? 'bg-rose-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                  >
                    <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-sm ${globalSettings.maintenance_mode ? 'left-7' : 'left-1'}`} />
                  </button>
                </div>

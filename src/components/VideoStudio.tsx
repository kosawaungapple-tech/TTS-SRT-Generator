...
export const VideoStudio: React.FC = () => {
  ...
  const startAnalysis = async () => {
    ...
    // 1. Check Cache
    const cacheKey = `vbs_cache_${videoFile.name}_${videoFile.size}`;
    const cached = localStorage.getItem(cacheKey);
    if (cached) {
      try {
        const { transcription, translation } = JSON.parse(cached);
        if (transcription && translation) {
          setTranscribedText(transcription);
          setTranslatedText(translation);
          setAnalysisStatus("ready");
          console.log("[VBS Video] Loaded from cache");
          // Re-generate audio if needed, or just let user click next
          // For now, if we have cache, we need to re-generate the TTS at least because it's not cached
        }
      } catch (e) {
        console.warn("[VBS Video] Cache parse error", e);
      }
    }
    ...
    const onRetry = (seconds: number, message: string) => {
      setAnalysisProgress(message);
    };

    try {
      // Step 0: Chunking using FFmpeg for stability
      setAnalysisProgress("ဗီဒီယိုကို အပိုင်းခွဲနေပါသည်... (Chunking)");
      const ffmpeg = ffmpegRef.current;
      ...
      await ffmpeg.exec(['-i', 'input.mp4', '-f', 'segment', '-segment_time', '30', '-c', 'copy', 'chunk_%03d.mp4']);
      
      // List chunks
      const fileList = await ffmpeg.listDir('.');
      const chunks: Blob[] = [];
      const chunkFiles = fileList
        .filter(f => f.name.startsWith('chunk_') && f.name.endsWith('.mp4'))
        .sort((a, b) => a.name.localeCompare(b.name));
      
      for (const cf of chunkFiles) {
        const cData = await ffmpeg.readFile(cf.name);
        chunks.push(new Blob([cData], { type: 'video/mp4' }));
        await ffmpeg.deleteFile(cf.name);
      }
      await ffmpeg.deleteFile('input.mp4');

      console.log(`[VBS Video] Created ${chunks.length} chunks for transcription`);

      // Step 1: Transcribe (with chunked processing inside service)
      setAnalysisProgress("အသံဖတ်နေသည်... (Transcribing Chunks)");
      const transcription = await gemini.transcribeVideoFile(videoFile, chunks, onRetry);
      setTranscribedText(transcription);
      ...
      localStorage.setItem(cacheKey, JSON.stringify({ transcription, translation }));

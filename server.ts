    ...
    try {
      ...
      if (!apiKey) {
        try {
          ...
          const maxAttempts = Math.min(totalKeys, 10);
          ...
          for (let i = 0; i < maxAttempts; i++) {
            ...
            try {
              ...
              console.warn(`[Proxy] Attempt ${i + 1}/${maxAttempts} failed: Status ${status}, Message: ${errMsg.substring(0, 100)}...`);

              // If it's a rate limit, overloaded, internal server error, or expired key error, try the next key
              if (status === 429 || status === 500 || status === 503 || status === 504 || status === 400 ||
                  (errMsg && (
                    errMsg.includes('RESOURCES_EXHAUSTED') || 
                ...
                console.warn(`[Proxy] Admin key failed (status ${status}), rotating to next...`);
                continue;
              }

    ...
    try {
      ...
      if (!apiKey) {
        try {
          ...
          let lastError = null;
          // Try up to 10 keys or all available to ensure we find a working one
          const totalKeys = adminKeys.length;
          const maxAttempts = Math.min(totalKeys, 15); // Increased attempts
          const startIndex = Math.floor(Math.random() * totalKeys);
          let currentTargetModel = model;
          ...
          for (let i = 0; i < maxAttempts; i++) {
            ...
            try {
              ...
              console.log(`[Proxy] Requesting model: ${currentTargetModel} with Admin Pool Key (${i + 1}/${maxAttempts})`);
              ...
              const errorObj = err as { status?: number; statusCode?: number; response?: { status: number, data?: unknown }; message?: string };
              ...
              const errData = JSON.stringify(errorObj.response?.data || errorObj);
              
              console.warn(`[Proxy] Admin key failed (i=${i}, status=${status}): ${errMsg.substring(0, 150)}`);

              // If it's a rate limit or internal error, rotate to next key
              const isQuotaError = errMsg.includes('RESOURCE_EXHAUSTED') || 
                                 errMsg.includes('quota') ||
                                 errData.includes('RESOURCE_EXHAUSTED') ||
                                 errData.includes('quota');
              
              const shouldRotate = status === 429 || status === 500 || status === 503 || status === 504 || isQuotaError;

              // TTS Fallback logic: If 3.1 TTS hits quota, fallback to 2.5 TTS
              if (isQuotaError && currentTargetModel === 'gemini-3.1-flash-tts-preview' && i >= 1) {
                console.warn(`[Proxy] gemini-3.1-flash-tts-preview quota hit. Switching fallback to gemini-2.5-flash-preview-tts.`);
                currentTargetModel = 'gemini-2.5-flash-preview-tts';
              }

              if (shouldRotate && i < maxAttempts - 1) {
                console.warn(`[Proxy] Rotating admin key...`);
                // For 429, wait a tiny bit to avoid simultaneous hammering if it's the same shared project
                if (status === 429) await new Promise(resolve => setTimeout(resolve, 500));
                continue;
              }
              
              // For other errors (like invalid prompt), or if we exhausted attempts, throw
              throw err;
            }
          }
